...
import { Loader2, Trophy, Users, BarChart3, Radio, Bell, BellOff, Goal, MapPin, Clock, Calendar, Building2 } from 'lucide-react';
...
export const LiveTournamentPage = () => {
  ...
  useEffect(() => {
    ...
    const matchChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'matches' }, (payload) => { handleMatchChange(payload); loadData(); })
      .subscribe();

    const teamsChannel = supabase
      .channel('live-standings')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'event_teams' }, () => { loadData(); })
      .subscribe();

    const goalsChannel = supabase
      .channel('live-goals')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'match_goals' }, () => { loadData(); })
      .subscribe();
  ...
  const loadData = async () => {
    try {
      ...
      const { data: liveMatchesCheck } = await supabase.from('matches').select('event_id').eq('status', 'in_progress').limit(1);
      ...
      if (!selectedEvent) {
        ...
        const upcoming = events.filter(e => e.date >= now).sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        if (upcoming.length > 0) selectedEvent = upcoming[0];
        isLive = false;
      }

      if (!selectedEvent) { setActiveEvent(null); setIsLiveEvent(false); setLoading(false); return; }

      setActiveEvent(selectedEvent);
      setIsLiveEvent(isLive);
      const activeEventId = selectedEvent.id;

      const { data: matchesData } = await supabase.from('matches').select('*').eq('event_id', activeEventId).order('match_date', { ascending: true });
      ...
      live.forEach(m => { playingTeamIds.add(m.home_team_id); playingTeamIds.add(m.away_team_id); });
      ...
      const { data: eventTeamsData } = await supabase.from('event_teams').select('*').eq('event_id', activeEventId).order('group_name').order('points', { ascending: false });
      setEventTeams((eventTeamsData || []) as EventTeam[]);

      // Load categories for event
      const { data: eventCatsData } = await supabase.from('event_categories').select('id, category_id').eq('event_id', activeEventId);
      if (eventCatsData && eventCatsData.length > 0) {
        ...
        // Map event_category_id → category info
  ...
  const getTeamLogo = (teamId: string) => teams.find(t => t.id === teamId)?.logo_url;
  ...
  const getMatchVenueInfo = (match: Match) => {
    ...
    if (!facility && !field) return null;
    return `${facility?.name || ''}${field ? ` · ${field.name}` : ''}`;
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <Card className="p-12 text-center max-w-md">
          ...
          <p className="text-muted-foreground">En este momento no hay ningún torneo en curso. Vuelve más tarde.</p>
        </Card>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-3">
              {isLiveEvent ? (
                ...
                <>
                  ...
                  <Badge className="bg-white/20 text-white border-white/30 text-lg px-4 py-1">PRÓXIMO EVENTO</Badge>
                </>
              )}
            </div>
            {isLiveEvent && (
                    ...
                    <div className="flex flex-col gap-1 mb-2">
                      ...
                      {venue && (
                        <p className="text-xs text-muted-foreground flex items-center gap-1">
                          <Building2 className="w-3 h-3" />{venue}
                        </p>
                      )}
                    </div>
                ...
                <Card key={match.id} className={`p-4 ${match.status === 'in_progress' ? 'border-2 border-red-500/50' : ''}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <span className={`font-medium ${liveTeamIds.has(match.home_team_id) ? 'text-red-600 dark:text-red-400' : ''}`}>{getTeamName(match.home_team_id)}</span>
                    </div>
                    <div className="text-2xl font-bold px-4">{match.home_score ?? 0} - {match.away_score ?? 0}</div>
                    <div className="flex items-center gap-2 flex-1 justify-end">
                      <span className={`font-medium ${liveTeamIds.has(match.away_team_id) ? 'text-red-600 dark:text-red-400' : ''}`}>{getTeamName(match.away_team_id)}</span>
